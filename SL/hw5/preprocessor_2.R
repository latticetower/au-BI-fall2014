...
check.connected <- function(digit) {
  ...
  colorer <- function(r, c, lvl = 0) {
    ...
    if (newdigit[r, c] == 0) {
        ...
        size <- size + colorer(rp1, cp1, lvl + 1)
        size <- size + colorer(rp1, cm1, lvl + 1)
        size <- size + colorer(rm1, cp1, lvl + 1)
        size <- size + colorer(rm1, cm1, lvl + 1)
    }
    size
  }
...
save_label_file('mnist/train-data.connected.comp.2', nrow(mnist.train), connected_comp.train)

save_label_file('mnist/test-data.connected.comp.2', nrow(mnist.test), connected_comp.test)
